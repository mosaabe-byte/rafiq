...
function environmentBlock(userEnv) {
  ...
  text += `\n\nتوثيق تلقائيّ: إن تأكّدتَ خلال المحادثة أنّ حالة أداةٍ تختلف عمّا هو مسجّل أعلاه (مثلاً كانت «مؤجّلة» فتبيّن أنّها مثبّتة، أو العكس)، أضِف في آخر ردّك — على سطر مستقلّ — وسماً بهذه الصيغة الحرفيّة: [[ENV:اسم_الأداة=installed]] أو [[ENV:اسم_الأداة=deferred]]. استعمل الأسماء: node، git، vscode، postgresql. لا تضع الوسم إلّا حين تتغيّر الحالة فعلاً عمّا هو مسجّل، ومرّة واحدة للأداة. لا تشرح الوسم للمستخدم — هو إشارة نظام.`;
  return text.trim();
}
